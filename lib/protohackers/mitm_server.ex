defmodule ThousandIsland.MITMHandler do
  @moduledoc """
  ...
  * Returning `{:error, reason, state}` will cause Thousand Island to close the socket & call the `c:handle_error/3` callback to
  ...
  defmacro __using__(_opts) do
    quote location: :keep do
      ...
      def handle_info({msg, client_socket, data}, {%{socket: client_socket} = socket, state})
          when msg in [:tcp, :ssl] do
        __MODULE__.handle_data(data, socket, state)
        |> handle_continuation(socket)
      end

      def handle_info({msg, client_socket}, {%{socket: client_socket} = socket, state})
          when msg in [:tcp_closed, :ssl_closed] do
        {:stop, {:shutdown, :peer_closed}, {socket, state}}
      end

      def handle_info({msg, client_socket, reason}, {%{socket: client_socket} = socket, state})
...
defmodule Protohackers.MITMServer do
  use ThousandIsland.MITMHandler

  @impl ThousandIsland.Handler
  def handle_connection(ti_socket, state) do
    {:ok, upstream_socket} =
      :gen_tcp.connect(state.upstream.host, state.upstream.port,
        mode: :binary,
        packet: :line,
        active: true
      )

    {
      :continue,
      state
      |> Map.put(:client_socket, ti_socket.socket)
      |> Map.put(:upstream_socket, upstream_socket)
    }
  end

  @impl ThousandIsland.Handler
  def handle_data(data, _ti_socket, state) do
  ...
  @impl ThousandIsland.Handler
  def handle_close(_ti_socket, state) do
